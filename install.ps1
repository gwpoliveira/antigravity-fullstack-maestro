...
if (-not (Test-Path $configDir)) {
    Write-Host "Criando diretório global do Antigravity em $configDir..." -ForegroundColor Yellow
...
Write-Host "📦 Copiando templates de produção..." -ForegroundColor Yellow
Copy-Item -Recurse -Force "$scriptRoot\templates\*" "$pluginDir\templates\"

Write-Host "⚙️ Registrando plugin global..." -ForegroundColor Yellow
...
# Copiar Skills base
$baseSkills = @('vps-nodocker-deploy', 'security-seal-audit', 'premium-ui-system')
foreach ($bs in $baseSkills) {
    $src = "$scriptRoot\.agents\skills\$bs\SKILL.md"
    if (Test-Path $src) {
        $pTarget = "$pluginDir\skills\$bs"
        $gTarget = "$skillsDir\$bs"
...
# Registrar Subagentes como Skills Invocáveis
$subagents = @(
    ...
    @{ name = 'subagent-security-guardian'; file = "$scriptRoot\.agents\subagents\subagent_security_guardian.md"; desc = 'Subagente auditor de segurança OWASP Top 10 e emissor do Selo de Segurança.' },
...
Write-Host "O Maestro e os 6 subagentes já estão ativos em qualquer projeto." -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
